<p>To add notes to a blog, drop an &#8220;info.txt&#8221; file in its directory.</p>
...
<?php foreach ($directories as $dir) : ?>
	...
	<tr<? if($d=="1") { echo " class='first'"; } ?>>
		<? if(substr($dir,0,1) == "_") { ?>
			<td colspan="4" class="notBlog"><a href="/blogs/<?php echo $dir ?>/"><?php echo $dir?></a></td>
		<? } elseif($dir != "detail.php") { ?>
			...
			<td class="thumb">
				...
				<? if($showThumb!="") { ?>
					<a href="/blogs/<?php echo $dir ?>/"><img width="50" height="50" alt="" src="/blogs/<? echo substr($showThumb,strlen(PATH)); ?>"></a>
				<? } else { /*?>
					<a href="detail.php?name=<?php echo $dir ?>"><img alt="[DIR]" src="/index/icons/folder_icon.gif" align="right" /></a>
				<? */ } ?>
			</td>

			<td class="filename"><b><a href="/blogs/<?php echo $dir ?>/"><?php echo $dir?></a></b></td>
			

			<td class="files"><? $note = PATH . "/" . $dir . "/" . "info.txt"; if ( file_exists($note) ) { echo "<pre>"; require($note); echo "</pre>"; } else { echo "<i class='noNotes'>No notes</i>"; } ?></td>
