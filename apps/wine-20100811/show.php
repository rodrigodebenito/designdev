 ...
 * @version $Id: show.php 69 2010-02-24 17:17:06Z nic.barajas $
 * @author jon
 */
...
<style>
	...
	.accordion_toggle span.rank {
		width: 18px;
	}
	
	.accordion_toggle span.name {
		width: 145px;
	}
...
for ($w=1; $w<=10; $w++) { 
	if($saved['fullname'.$w]) {
      $emailfullname = str_replace($oldamp, $newamp, $saved['fullname'.$w]);
			echo $w.". ".$emailfullname.", ".$saved['rating'.$w].", $".$saved['price'.$w]."%0D%0A";
			echo $saved['desc'.$w];
