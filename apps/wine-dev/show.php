...
?>%0AFor more information about these wines and to read the whole article, go to:%0Ahttp://www.nytimes.com/2009/02/18/dining/reviews/18wine.html">E-Mail This List</a>
...
<? for ($w=1; $w<=10; $w++) { 
	if($saved['shortname'.$w]) { ?>
<li class="<? if($saved['bestvalue'.$w]=="1") { $didBestvalue="1"; echo "bestvalue"; } ?><? if($w==10) { echo ' last'; } ?>">
<a class="accordion_toggle"> <em><? echo $w; ?>.</em> <? echo $saved['shortname'.$w]; ?></a>
...
<? if( strlen($saved['proof'.$w]) >=1 ) { ?> alc. <? echo $saved['proof'.$w]; ?>% vol.<? } ?>
	...
	<div class="image">
		<img width="190" border="0" alt="" src="<? echo $saved['photo'.$w]; ?>">
	...
	<? if( strlen($saved['look'.$w]) >=1 ) { ?>
	<p>Look for this wine on: <a href="<? echo $saved['lookurl'.$w]; ?>"><? echo $saved['look'.$w]; ?></a></p>
	<? } ?>			
	...
	<ul class="wineTools">
		<li class="email last">
			...
			?>%0A--%0A%0AFor the complete list of wines and to read the whole article, go to:%0Ahttp://www.nytimes.com/2009/02/18/dining/reviews/18wine.html">E-Mail Wine</a>
		</li>
		<? // <li class="mobile"><a href="#">Send Wine to Mobile</a></li> ?>
	</ul>
</div>
</li>
